...
class EditFiles extends Page
{
    ...
    public function mount(string $path): void
    {
        ...
        if (str($path)->endsWith('.coramtixignore')) {
            AlertBanner::make('.coramtixignore_info')
                ->title(trans('server/file.alerts.coramtixignore.title'))
                ->body(trans('server/file.alerts.coramtixignore.body'))
                ->info()
                ->closable()
                ->send();
        }
