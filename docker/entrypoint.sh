...
if [ -f /var/www/html/.env ]; then
  ...
  if [ -z $APP_KEY ]; then
     ...
     echo -e "APP_KEY=$APP_KEY" > /coramtix-data/.env
  else
  ...
  ## enable installer
  echo -e "APP_INSTALLED=false" >> /coramtix-data/.env
fi

sed -i "s/upload_max_filesize = 2M/upload_max_filesize = ${UPLOAD_LIMIT}M/" /usr/local/etc/php/php.ini-production

mkdir -p /coramtix-data/database /coramtix-data/storage/avatars /coramtix-data/storage/fonts /var/www/html/storage/logs/supervisord 2>/dev/null
